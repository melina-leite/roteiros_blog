...
<!---
AINDA PRECISA COLOCAR OS LINKS DE ROTEIROS ANTERIORES - normalidade multivariada
--->

Muitos testes de hipótese multivariados (ver [este roteiro]()) baseiam-se na homocedasticidade das matrizes de covariância, que é o análogo multivariado da homogeneidade de variâncias em testes de hipótese univariados (por exemplo, usando o teste de [Levene](https://en.wikipedia.org/wiki/Levene%27s_test) para avaliar homocedasticidade univariada). 
...
Podemos também comparar a homocedasticidade das matrizes de covariância com uma generalização do teste de homogeneidade de variâncias de Levene proposto por Anderson (2006). Esta proposta está baseada na distância das observações aos centróides dos grupos no espaço multivariado. Para testar se as variâncias dos grupos são diferentes, as distâncias entre as observações dos grupos ao centróide do grupo são comparadas por ANOVA. Quando usamos a distância euclidiana, temos o análogo multivariado do teste de Levene (mas é possível usar outras distâncias/dissimilaridades).
...
```{r}
library(vegan)
mod <- betadisper(dist(pardais[,-1], "euclidian"), pardais[,1])
anova(mod)
```

O resultado deste método também indica que podemos afirmar homocedasticidade entre as matrizes de covariância para os pardais sobreviventes e não sobreviventes. 
