...
<!---
DÚVIDAS PARA PROFA JULIA

- QUAL O ALPHA GERALMENTE SE ESCOLHE PARA DETECTAR OUTLIERS? 0.9, 0.95, 0.5?

-  HÁ VANTAGENS EM SE USAR UMA ESTIMAÇÃO ROBUSTA PARA CALCULAR AS DISTÂNCIAS DE MAHALANOBIS, COMO FEITO NO mvOutlier()
"Robust distances are estimated from minimum covariance determinant estimators rather than the sample covariance [29]."
...
Escolhemos duas variáveis do conjunto de dados `swiss` do pacote `datasets`[^1] e vamos primeiro inspecioná-las separadamente com os boxplots univariados.
...
No boxplot bivariado, detectamos os mesmos _outliers_ encontrados para `Education`.

A desvantagem dos boxplots bivariados é que eles avaliam os outliers apenas para duas variáveis por vez. A seguir veremos uma forma mais geral de se avaliar os valores extremos multivariados.
...
Desta forma, para o critério usando a distância de Mahalanobis, apenas 3 observações foram consideradas _outliers_ bivariados. Este critério está dizendo que as 3 observações com maior distância tem pouca probabilidade de serem advindas da população com distribuição normal bivariada assumida nos dados.
...
Com mais variáveis, tivemos diferentes observações detectadas como _outliers_ (exceto `V. De Geneve` que foi detectada em todos os métodos) e por isso a importância de se analisar os valores extremos em dados multivariados de maneira conjunta!

## Usando pacote `MVN

O pacote `MVN` possui uma função que também detecta _outliers_ multivariados a partir da **distância de Mahalanobis**. Esse pacote usa dois métodos chamados de _robustos_ (ver detales [aqui](https://pdfs.semanticscholar.org/5508/25b2681f1cc067c66df6ddbd62c74b441869.pdf?_ga=2.25175998.286279602.1517010562-1528935492.1517010562)) para calcular a distância de Mahalanobis (a distância de Mahalanobis Robusta e a Ajustada), e portanto dão resultados um pouco diferentes do que fizemos anteriormente. 
