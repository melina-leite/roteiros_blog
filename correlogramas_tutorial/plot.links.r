...
# Melina modificou em março 2017 para haver a opção de colocar ou não o texto de cada ponto
#
{
...
if (text == TRUE) text(XY, labels=rownames(XY), pos=2)
title(main = c("Linkage map","D â‰¤",thresh), family="serif")
n = nrow(XY)
for(j in 1:(n-1)) {
   for(jj in (j+1):n) {
      if((D.mat[j,jj]<=thresh)&(D.mat[j,jj]>0)) lines(c(XY[j,1], XY[jj,1]), c(XY[j,2], XY[jj,2]))
      }
   }
}